...
def main():
    ...
    has_update = args.lang or getattr(args, 'no_translate', False) or getattr(args, 'translate', False)
    if not has_update:
        print(json.dumps({'error': 'At least one option required (--lang, --no-translate, or --translate)'}))
        sys.exit(1)
